...
abstract class DifferentTouchInput {
	public interface OnInputEventListener {
		...
		public void onMotionEvent(int x, int y, int action, int pointerId,
				int pressure, int radius, int touchCount);
	...
	public static DifferentTouchInput getInstance() {
		...
		try {
			if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.ICE_CREAM_SANDWICH) {
				if (android.os.Build.MODEL.equals("GT-N7000")
						|| android.os.Build.MODEL.equals("SGH-I717"))
	...
	private static class SingleTouchInput extends DifferentTouchInput {
		...
		public void process(final MotionEvent event) {
			...
			if (action >= 0) {
				if (mOnInputEventListener != null)
					mOnInputEventListener.onMotionEvent((int) event.getX(),
							(int) event.getY(), action, 0,
							(int) (event.getPressure() * 1000.0),
							(int) (event.getSize() * 1000.0), 1);
			}
		}
	}
		...
		public void process(final MotionEvent event) {
			...
			if ((event.getAction() & MotionEvent.ACTION_MASK) == MotionEvent.ACTION_UP
				...
				for (int i = 0; i < TOUCH_EVENTS_MAX; i++) {
					if (touchEvents[i].down) {
						...
						if (mOnInputEventListener != null)
							mOnInputEventListener.onMotionEvent(
									touchEvents[i].x, touchEvents[i].y, action,
									i, touchEvents[i].pressure,
									touchEvents[i].size,
			...
			if ((event.getAction() & MotionEvent.ACTION_MASK) == MotionEvent.ACTION_DOWN) {
				...
				for (int i = 0; i < event.getPointerCount(); i++) {
					...
					if (mOnInputEventListener != null)
						mOnInputEventListener.onMotionEvent(touchEvents[id].x,
			...
			if ((event.getAction() & MotionEvent.ACTION_MASK) == MotionEvent.ACTION_MOVE
				...
				/*
				 * String s = "MOVE: ptrs " + event.getPointerCount(); for( int
				 * i = 0 ; i < event.getPointerCount(); i++ ) { s += " p" +
				 * event.getPointerId(i) + "=" + (int)event.getX(i) + ":" +
																								...
																								// 3.1
			{
				...
				if (mOnInputEventListener != null)
					mOnInputEventListener.onMotionEvent(touchEvents[0].x,
	...
	private static class XperiaPlayTouchpadTouchInput extends MultiTouchInput {
		...
		XperiaPlayTouchpadTouchInput() {
			...
			for (int id : devIds) {
				...
				if (device == null)
					continue;
				System.out.println("Engine: input device ID " + id + " type "
						+ device.getSources() + " name " + device.getName());
				...
				if (range != null) {
					...
					System.out.println("Engine: touch pad X range " + xmin
							+ ":" + xmax);
				}
				...
				if (range != null) {
					...
					System.out.println("Engine: touch pad Y range " + ymin
							+ ":" + ymax);
				}
				// Xperia Play has long wide touchpad with joystick-like
				// embossing on the sides, so we'll leave only a left joystick
				// to function
				// I don't know how to use the second joystick, so I'll just
				// ignore it for now
				minRange = Math.min(Math.abs(ymax - ymin),
						Math.abs(xmax - xmin));
		...
		public void process(final MotionEvent event) {
			boolean hwMouseEvent = (event.getSource() == InputDevice.SOURCE_MOUSE
					|| event.getSource() == InputDevice.SOURCE_STYLUS || (event
					.getMetaState() & KeyEvent.FLAG_TRACKING) != 0); // Hack to
																		// recognize
																		// Galaxy
																		// Note
																		// Gingerbread
		...
		public void processGenericEvent(final MotionEvent event) {
			...
			/*
			 * int x = (int)((event.getX() - xmin) / xmax * 65535.0f); int y =
			 * (int)((event.getY() - ymin) / ymax * 65535.0f);
			 */
			// Use only right square part of a touch surface - I've heard
			// reports that it breaks functionality, feel free to uncomment and
			// test it.
			...
			// " action " + event.getAction() + " down " + down + " multitouch "
			// + multitouch );

			// DemoGLSurfaceView.nativeTouchpad( x, 65535 - y, down, multitouch
			// ); // Y axis is inverted, as you may have guessed
		}
	}
			...
			if (buttonStateNew != buttonState) {
				for (int i = 1; i <= MotionEvent.BUTTON_FORWARD; i *= 2) {
					if ((buttonStateNew & i) != (buttonState & i))
						if (mOnInputEventListener != null)
							mOnInputEventListener.onMouseButtonEvent(i,
	...
	private static class GalaxyNoteIcsTouchInput extends IcsTouchInput {
		...
		public void process(final MotionEvent event) {
			// HACK for Galaxy Note stylus, which pushes the cursor to the
			// lower-right part of the screen, when you lift the stylus.
			// Also it reports the stylus as the mouse
			if (!(event.getSource() == InputDevice.SOURCE_MOUSE
					&& (int) event.getX() == 0 && (int) event.getY() == 799))
				super.process(event);
		}
	}
}
