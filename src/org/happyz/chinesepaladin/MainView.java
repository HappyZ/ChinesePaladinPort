...
public class MainView extends RelativeLayout {
	...
	public boolean onGenericMotionEvent(final MotionEvent event) {
		if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.GINGERBREAD) {
			if (AndroidGingerBreadAPI.MotionEventGetSource(event) == InputDevice.SOURCE_JOYSTICK) {
				...
				float x = AndroidGingerBreadAPI.MotionEventGetAxisValue(event,
						MotionEvent.AXIS_X);
				float y = AndroidGingerBreadAPI.MotionEventGetAxisValue(event,
						MotionEvent.AXIS_Y);
				...
				for (int i = 0; diff != 0; diff >>= 1, state >>= 1, i++) {
					if ((diff & 1) != 0) {
						if (mTouchMode != null) {
							mTouchMode.onKeyEvent(
									Globals.JOYSTICK_AXIS_KEY_ARRAY[i],
									((state & 1) != 0 ? 1 : 0));
						}
					}
				}
				...
				float hatx = AndroidGingerBreadAPI.MotionEventGetAxisValue(
						event, MotionEvent.AXIS_HAT_X);
				float haty = AndroidGingerBreadAPI.MotionEventGetAxisValue(
						event, MotionEvent.AXIS_HAT_Y);
				...
				for (int i = 0; diff != 0; diff >>= 1, state >>= 1, i++) {
					if ((diff & 1) != 0) {
						if (mTouchMode != null) {
							mTouchMode.onKeyEvent(
									Globals.JOYSTICK_AXISHAT_KEY_ARRAY[i],
									((state & 1) != 0 ? 1 : 0));
						}
					}
				}
	...
	private static final int MENU_ITEM_ID_USER_MENU_KEY_LAST = MENU_ITEM_ID_USER_MENU_KEY_FIRST
			+ Globals.MENU_KEY_NUM - 1;

	private static final int MENU_ITEM_ID_USER_SUBMENU_KEY_FIRST = Menu.FIRST + 200;
	private static final int MENU_ITEM_ID_USER_SUBMENU_KEY_LAST = MENU_ITEM_ID_USER_SUBMENU_KEY_FIRST
			+ Globals.SUBMENU_KEY_NUM - 1;
	...
	private static final int MENU_ITEM_ID_SETTING_BUTTON_LEFT_NUM_LAST = MENU_ITEM_ID_SETTING_BUTTON_LEFT_NUM_FIRST
			+ Globals.BUTTON_LEFT_MAX;
	...
	private static final int MENU_ITEM_ID_SETTING_BUTTON_RIGHT_NUM_LAST = MENU_ITEM_ID_SETTING_BUTTON_RIGHT_NUM_FIRST
			+ Globals.BUTTON_RIGHT_MAX;
	...
	private static final int MENU_ITEM_ID_SETTING_BUTTON_TOP_NUM_LAST = MENU_ITEM_ID_SETTING_BUTTON_TOP_NUM_FIRST
			+ Globals.BUTTON_TOP_MAX;
	...
	private static final int MENU_ITEM_ID_SETTING_BUTTON_BOTTOM_NUM_LAST = MENU_ITEM_ID_SETTING_BUTTON_BOTTOM_NUM_FIRST
			+ Globals.BUTTON_BOTTOM_MAX;
	...
	public boolean onPrepareOptionsMenu(Menu menu) {
		...
		for (int i = 0; i < Globals.MENU_KEY_NUM; i++) {
			Integer sdlKey = (Integer) Globals.SDLKeyAdditionalKeyMap
					.get(Integer.valueOf(Globals.MENU_KEY_ARRAY[i]));
			if (sdlKey != null
					&& sdlKey.intValue() != SDL_1_2_Keycodes.SDLK_UNKNOWN) {
				String name = Globals.SDLKeyFunctionNameMap.get(sdlKey);
				if (name != null) {
					menu.add(Menu.NONE, (MENU_ITEM_ID_USER_MENU_KEY_FIRST + i),
							Menu.NONE, name);
				}
			}
		}
		...
		SubMenu menu_userfunc = menu.addSubMenu(getResources().getString(
				R.string.function));
		for (int i = 0; i < Globals.SUBMENU_KEY_NUM; i++) {
			Integer sdlKey = (Integer) Globals.SDLKeyAdditionalKeyMap
					.get(Integer.valueOf(Globals.SUBMENU_KEY_ARRAY[i]));
			if (sdlKey != null
					&& sdlKey.intValue() != SDL_1_2_Keycodes.SDLK_UNKNOWN) {
				String name = Globals.SDLKeyFunctionNameMap.get(sdlKey);
				if (name != null) {
					menu_userfunc.add(Menu.NONE,
							(MENU_ITEM_ID_USER_SUBMENU_KEY_FIRST + i),
							Menu.NONE, name);
				}
			}
		}
		...
		SubMenu menu_mousecursor = menu.addSubMenu(getResources().getString(
				R.string.mouse_cursor));
		menu_mousecursor.add(Menu.NONE, MENU_ITEM_ID_SETTING_MOUSECURSOR_SHOW,
				Menu.NONE, getResources().getString(R.string.show));
		menu_mousecursor.add(Menu.NONE, MENU_ITEM_ID_SETTING_MOUSECURSOR_HIDE,
				Menu.NONE, getResources().getString(R.string.hide));

		if (Globals.BUTTON_USE) {
			SubMenu menu_btn_left = menu.addSubMenu(getResources().getString(
					R.string.button_left));
			if (Globals.ButtonLeftEnabled) {
				menu_btn_left.add(Menu.NONE,
						MENU_ITEM_ID_SETTING_BUTTON_LEFT_ENABLED_DISABLE,
						Menu.NONE, getResources().getString(R.string.disable));
				for (int i = 1; i <= Globals.BUTTON_LEFT_MAX; i++) {
					menu_btn_left.add(Menu.NONE,
							(MENU_ITEM_ID_SETTING_BUTTON_LEFT_NUM_FIRST + i),
							Menu.NONE, "" + i + "unit");
				}
			} else {
				menu_btn_left.add(Menu.NONE,
						MENU_ITEM_ID_SETTING_BUTTON_LEFT_ENABLED_ENABLE,
						Menu.NONE, getResources().getString(R.string.enable));
			}

			SubMenu menu_btn_right = menu.addSubMenu(getResources().getString(
					R.string.button_right));
			if (Globals.ButtonRightEnabled) {
				menu_btn_right.add(Menu.NONE,
						MENU_ITEM_ID_SETTING_BUTTON_RIGHT_ENABLED_DISABLE,
						Menu.NONE, getResources().getString(R.string.disable));
				for (int i = 1; i <= Globals.BUTTON_RIGHT_MAX; i++) {
					menu_btn_right.add(Menu.NONE,
							(MENU_ITEM_ID_SETTING_BUTTON_RIGHT_NUM_FIRST + i),
							Menu.NONE, "" + i + " unit");
				}
			} else {
				menu_btn_right.add(Menu.NONE,
						MENU_ITEM_ID_SETTING_BUTTON_RIGHT_ENABLED_ENABLE,
						Menu.NONE, getResources().getString(R.string.enable));
			}

			SubMenu menu_btn_top = menu.addSubMenu(getResources().getString(
					R.string.button_top));
			if (Globals.ButtonTopEnabled) {
				menu_btn_top.add(Menu.NONE,
						MENU_ITEM_ID_SETTING_BUTTON_TOP_ENABLED_DISABLE,
						Menu.NONE, getResources().getString(R.string.disable));
				for (int i = 1; i <= Globals.BUTTON_TOP_MAX; i++) {
					menu_btn_top.add(Menu.NONE,
							(MENU_ITEM_ID_SETTING_BUTTON_TOP_NUM_FIRST + i),
							Menu.NONE, "" + i + "unit");
				}
			} else {
				menu_btn_top.add(Menu.NONE,
						MENU_ITEM_ID_SETTING_BUTTON_TOP_ENABLED_ENABLE,
						Menu.NONE, getResources().getString(R.string.enable));
			}

			SubMenu menu_btn_bottom = menu.addSubMenu(getResources().getString(
					R.string.button_bottom));
			if (Globals.ButtonBottomEnabled) {
				menu_btn_bottom.add(Menu.NONE,
						MENU_ITEM_ID_SETTING_BUTTON_BOTTOM_ENABLED_DISABLE,
						Menu.NONE, getResources().getString(R.string.disable));
				for (int i = 1; i <= Globals.BUTTON_BOTTOM_MAX; i++) {
					menu_btn_bottom.add(Menu.NONE,
							(MENU_ITEM_ID_SETTING_BUTTON_BOTTOM_NUM_FIRST + i),
							Menu.NONE, "" + i + "unit");
				}
			} else {
				menu_btn_bottom.add(Menu.NONE,
						MENU_ITEM_ID_SETTING_BUTTON_BOTTOM_ENABLED_ENABLE,
						Menu.NONE, getResources().getString(R.string.enable));
			}

			SubMenu menu_video_xmargin = menu
					.addSubMenu(R.string.video_x_margin);
			for (int i = 0; i <= (MENU_ITEM_ID_SETTING_VIDEO_XMARGIN_LAST - MENU_ITEM_ID_SETTING_VIDEO_XMARGIN_FIRST); i++) {
				menu_video_xmargin.add(Menu.NONE,
						(MENU_ITEM_ID_SETTING_VIDEO_XMARGIN_FIRST + i),
						Menu.NONE, "" + i);
			}

			SubMenu menu_video_ymargin = menu.addSubMenu(getResources()
					.getString(R.string.video_y_margin));
			for (int i = 0; i <= (MENU_ITEM_ID_SETTING_VIDEO_YMARGIN_LAST - MENU_ITEM_ID_SETTING_VIDEO_YMARGIN_FIRST); i++) {
				menu_video_ymargin.add(Menu.NONE,
						(MENU_ITEM_ID_SETTING_VIDEO_YMARGIN_FIRST + i),
						Menu.NONE, "" + i);
			}
		}

		SubMenu menu_video_xpos = menu.addSubMenu(getResources().getString(
				R.string.video_x_pos));
		menu_video_xpos.add(Menu.NONE,
				MENU_ITEM_ID_SETTING_VIDEO_XPOSITION_LEFT, Menu.NONE,
				getResources().getString(R.string.left));
		menu_video_xpos.add(Menu.NONE,
				MENU_ITEM_ID_SETTING_VIDEO_XPOSITION_CENTER, Menu.NONE,
				getResources().getString(R.string.center));
		menu_video_xpos.add(Menu.NONE,
				MENU_ITEM_ID_SETTING_VIDEO_XPOSITION_RIGHT, Menu.NONE,
				getResources().getString(R.string.right));

		SubMenu menu_video_ypos = menu.addSubMenu(getResources().getString(
				R.string.video_y_pos));
		menu_video_ypos.add(Menu.NONE,
				MENU_ITEM_ID_SETTING_VIDEO_YPOSITION_TOP, Menu.NONE,
				getResources().getString(R.string.top));
		menu_video_ypos.add(Menu.NONE,
				MENU_ITEM_ID_SETTING_VIDEO_YPOSITION_CENTER, Menu.NONE,
				getResources().getString(R.string.center));
		menu_video_ypos.add(Menu.NONE,
				MENU_ITEM_ID_SETTING_VIDEO_YPOSITION_BOTTOM, Menu.NONE,
				getResources().getString(R.string.bottom));

		SubMenu menu_gamepad_position = menu.addSubMenu(getResources()
				.getString(R.string.gamepad_pos));
		menu_gamepad_position.add(Menu.NONE,
				MENU_ITEM_ID_SETTING_GAMEPAD_POSITION_FIRST, Menu.NONE, "0 ("
						+ getResources().getString(R.string.top) + ")");
		for (int i = 1; i < (MENU_ITEM_ID_SETTING_GAMEPAD_POSITION_LAST - MENU_ITEM_ID_SETTING_GAMEPAD_POSITION_FIRST); i++) {
			menu_gamepad_position.add(Menu.NONE,
					(MENU_ITEM_ID_SETTING_GAMEPAD_POSITION_FIRST + i),
					Menu.NONE, "" + (i * 10));
		}
		menu_gamepad_position.add(Menu.NONE,
				MENU_ITEM_ID_SETTING_GAMEPAD_POSITION_LAST, Menu.NONE, "100 ("
						+ getResources().getString(R.string.bottom) + ")");

		SubMenu menu_gamepad_size = menu.addSubMenu(getResources().getString(
				R.string.gamepad_size));
		for (int i = 1; i <= (MENU_ITEM_ID_SETTING_GAMEPAD_SIZE_LAST - MENU_ITEM_ID_SETTING_GAMEPAD_SIZE_FIRST); i++) {
			menu_gamepad_size.add(Menu.NONE,
					(MENU_ITEM_ID_SETTING_GAMEPAD_SIZE_FIRST + i), Menu.NONE,
					"" + (i * 10) + "%");
		}

		SubMenu menu_gamepad_opacity = menu.addSubMenu(getResources()
				.getString(R.string.gamepad_opacity));
		for (int i = 0; i <= (MENU_ITEM_ID_SETTING_GAMEPAD_OPACITY_LAST - MENU_ITEM_ID_SETTING_GAMEPAD_OPACITY_FIRST); i++) {
			menu_gamepad_opacity.add(Menu.NONE,
					(MENU_ITEM_ID_SETTING_GAMEPAD_OPACITY_FIRST + i),
					Menu.NONE, "" + (i * 10) + "%");
		}

		SubMenu menu_gamepad_arrow_button = menu.addSubMenu(getResources()
				.getString(R.string.gamepad_arrow_button));
		...
		SubMenu menu_appconfig = menu.addSubMenu(getResources().getString(
				R.string.app_launch_config));
		if (Locals.AppLaunchConfigUse) {
			menu_appconfig.add(Menu.NONE,
					MENU_ITEM_ID_SETTING_APPCONFIG_NOTUSE, Menu.NONE,
					getResources().getString(R.string.disable));
		} else {
			menu_appconfig.add(Menu.NONE, MENU_ITEM_ID_SETTING_APPCONFIG_USE,
					Menu.NONE, getResources().getString(R.string.enable));
		}
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		if (d >= (MENU_ITEM_ID_USER_MENU_KEY_FIRST)
				&& d <= (MENU_ITEM_ID_USER_MENU_KEY_LAST)) {
			int index = d - MENU_ITEM_ID_USER_MENU_KEY_FIRST;
			int key = Globals.MENU_KEY_ARRAY[index];
			nativeKey(key, 1);
			nativeKey(key, 0);
		} else if (d >= (MENU_ITEM_ID_USER_SUBMENU_KEY_FIRST)
				&& d <= (MENU_ITEM_ID_USER_SUBMENU_KEY_LAST)) {
			...
			Settings.SaveLocals(mActivity);
			update();
		} else if (d >= MENU_ITEM_ID_SETTING_VIDEO_YMARGIN_FIRST
				&& d <= MENU_ITEM_ID_SETTING_VIDEO_YMARGIN_LAST) {
			...
			Settings.SaveGlobals(mActivity);
			update();
		} else if (d >= MENU_ITEM_ID_SETTING_BUTTON_LEFT_NUM_FIRST
				&& d <= MENU_ITEM_ID_SETTING_BUTTON_LEFT_NUM_LAST) {
			Globals.ButtonLeftNum = d
					- MENU_ITEM_ID_SETTING_BUTTON_LEFT_NUM_FIRST;
			Settings.SaveGlobals(mActivity);
			update();
			...
			Settings.SaveGlobals(mActivity);
			update();
		} else if (d >= MENU_ITEM_ID_SETTING_BUTTON_RIGHT_NUM_FIRST
				&& d <= MENU_ITEM_ID_SETTING_BUTTON_RIGHT_NUM_LAST) {
			Globals.ButtonRightNum = d
					- MENU_ITEM_ID_SETTING_BUTTON_RIGHT_NUM_FIRST;
			Settings.SaveGlobals(mActivity);
			update();
			...
			Settings.SaveGlobals(mActivity);
			update();
		} else if (d >= MENU_ITEM_ID_SETTING_BUTTON_TOP_NUM_FIRST
				&& d <= MENU_ITEM_ID_SETTING_BUTTON_TOP_NUM_LAST) {
			Globals.ButtonTopNum = d
					- MENU_ITEM_ID_SETTING_BUTTON_TOP_NUM_FIRST;
			Settings.SaveGlobals(mActivity);
			update();
			...
			Settings.SaveGlobals(mActivity);
			update();
		} else if (d >= MENU_ITEM_ID_SETTING_BUTTON_BOTTOM_NUM_FIRST
				&& d <= MENU_ITEM_ID_SETTING_BUTTON_BOTTOM_NUM_LAST) {
			Globals.ButtonBottomNum = d
	...
	public void setMouseCursorRGB(int fillColorR, int fillColorG,
			int fillColorB, int strokeColorR, int strokeColorG, int strokeColorB) {
		mMouseCursor.setMouseCursorRGB(fillColorR, fillColorG, fillColorB,
				strokeColorR, strokeColorG, strokeColorB);
	}

	public void updateMouseCursor() {
		LayoutParams params = new LayoutParams(MouseCursor.WIDTH, MouseCursor.HEIGHT);
		params.setMargins(mGLViewX + mMousePointX,mGLViewY + mMousePointY,0,0);
		mMouseCursor.setLayoutParams(params);
	...
	public void setScreenOrientation(int orientation) {
		 ...
		 * mActivity.setRequestedOrientation(
		 * ActivityInfo.SCREEN_ORIENTATION_REVERSE_LANDSCAPE ); } else { if(
		 * android.os.Build.VERSION.SDK_INT >=
		 * android.os.Build.VERSION_CODES.GINGERBREAD ){ if(
		 * ActivityInfo.SCREEN_ORIENTATION_REVERSE_PORTRAIT ){
		 * mActivity.setRequestedOrientation(
		 * ActivityInfo.SCREEN_ORIENTATION_REVERSE_PORTRAIT ); } else {
		 * mActivity.setRequestedOrientation(
		 * ActivityInfo.SCREEN_ORIENTATION_REVERSE_LANDSCAPE ); } } else {
		 * mActivity.setRequestedOrientation(
	...
	private class OnClickListenerForKeyConfigDialog implements
		...
		public OnClickListenerForKeyConfigDialog(MainView mainView,
				int javaKeyCode, int[] sdlKeyArray,
				String[] sdlKeyFunctionNameArray) {
			mMainView = mainView;
		...
		public void onClick(DialogInterface dialog, int whichItem) {
			...
			Toast.makeText(getActivity(),
					"Set:" + mSdlKeyFunctionNameArray[whichItem],
	...
	public int nativeKey(int keyCode, int down) {
		if (!mIsKeyConfigMode) {
			...
			AlertDialog.Builder alertDialogBuilder = new AlertDialog.Builder(
					getActivity());
			alertDialogBuilder.setTitle(getResources().getString(
					R.string.select_key_func)
					+ " : " + keyCode);
			alertDialogBuilder.setItems(nameArray,
					new OnClickListenerForKeyConfigDialog(this, keyCode,
							keyArray, nameArray));
			alertDialogBuilder.setCancelable(true);
	...
	public void enterKeyConfigMode() {
		...
		mKeyConfigTextView.setText(getResources().getString(
				R.string.key_cfg_mode));
		mKeyConfigTextView.setTextColor(Color.WHITE);
		mKeyConfigTextView.setBackgroundColor(Color.BLACK);
		

		LayoutParams params = new LayoutParams(mGLViewWidth, LayoutParams.WRAP_CONTENT);
		params.setMargins(mGLViewX,mGLViewY,0,0);

		addView(mKeyConfigTextView, new RelativeLayout.LayoutParams(params));

		Toast.makeText(getActivity(),
				getResources().getString(R.string.press_key_btn),
				Toast.LENGTH_LONG).show();
	}
